...
class MyApp:
    def __init__(self, root):
        ...
        self.menu.add_command(label="Качество", command=self.open_quality)
        self.menu.add_command(label="Отчетность", command=self.open_reporting)
        ...
        # Пример новостей
        self.news_blocks = [
            f"Успешно завершено производство новой партии метел. Все изделия прошли контроль качества.",
            f"Запущен новый проект по улучшению экологичности упаковки продукции.",
            f"Введены новые стандарты качества для древесины, используемой в производстве метел.",
    ...
    def create_technical_tables(self):
        ...
        prod_data = [
            ("Производительность", "1200 единиц/день"),
            ("Уровень брака", "2%"),
            ("Среднее время простоя", "1 час/неделя"),
            ("Количество заказов в день", "200"),
            ("Средняя скорость производства", "500 единиц/час"),
            ("Процент выполненных заказов", "95%"),
            ("Общее время работы", "16 часов/день"),
            ("Количество сотрудников", "50"),
            ("Нагрузка на оборудование", "75%"),
            ("Процент выполнения плана", "90%"),
            ("Среднее время выполнения заказа", "3 часа"),
        ]
    ...
    def update_status_bars(self):
        """Обновляет статус-бары каждые 2 секунды."""
        for i in range(8):
    ...
    def open_quality(self):
        messagebox.showinfo("Качество", "Открыта вкладка Качество")

    def open_reporting(self):
        messagebox.showinfo("Отчетность", "Открыта вкладка Отчетность")
